...
export class FugueRuntimeService {
  constructor() {
    const imports = {
      "env": new Proxy({
        wasm_set_tallocated: (p: bigint) => { this.tempRegister = p },
  ...
  loadProgram(src: string): [boolean, string, string] {
    if (!this.wasmLoaded) return [false, '', ''];
    
    this.findExport('talloc_reg')(this.jaiContext);
    const jaiStr    = this.toJaiString(src);
    const tempReset = this.findExport('reset_temporary_storage');
    
    this.findExport('load')(this.jaiContext, jaiStr, this.tempRegister);
    if (this.copyJaiMemory(this.tempRegister, 8n).getBigInt64(0) === 0n) {
      tempReset(this.jaiContext);
      return [false, '', ''];
    }
    
    this.findExport('get_inst')(this.jaiContext, this.tempRegister);
    const instructions = this.toJsString(this.tempRegister);
    
    this.findExport('run_main')(this.jaiContext, this.tempRegister);
    const output = this.toJsString(this.tempRegister);
    
    tempReset(this.jaiContext);
    return [true, instructions, output];
  }
  
  private wasmLoaded   : boolean = false;
  private wasmModule   : WebAssembly.Instance | undefined;
  private jaiContext   : bigint = 0n;
  private tempRegister : bigint = 0n;
  ...
  private toJaiString(str: string): bigint {
    if (str !== '') {
      // out parameter for talloc and make_string
      this.findExport("talloc_reg")(this.jaiContext);
      // allocate space for string
      // at most 4 bytes per UTF-8 code point
      let count = BigInt(str.length << 2);
      this.findExport("talloc")(this.jaiContext, count, this.tempRegister);
      const data = this.copyJaiMemory(this.tempRegister, 8n).getBigInt64(0);
      count = this.stringToUTF8(str, data, count);
      
      // construct the string, also uses temporary storage
      this.findExport("make_string")(this.jaiContext, count, data, this.tempRegister);
      return this.copyJaiMemory(this.tempRegister, 8n).getBigInt64(0);
    }
    return 0n;
  }
